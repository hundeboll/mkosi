...
:   Whether to enforce or disable verity for extension images. Takes one of
    `no`, `signed, `hash` and `auto`. If set to `signed`, a verity key and
    certificate must be present and the build will fail if we don't detect any
    verity partitions in the disk image produced by **systemd-repart**. If set
    to `no`, verity partitions will be excluded from the extension images
